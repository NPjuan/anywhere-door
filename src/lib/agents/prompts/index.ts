...
⚠️ 返程时间约束（极其重要）：
如果 prompt 中有 [返程时间] 字段，最后一天的所有活动必须在「返程起飞时间 - 2小时」前结束。
如果返程起飞时间在凌晨（00:00-05:59），最后一天的活动最晚安排到前一晚 22:00，不得安排凌晨游览。
绝对禁止在飞机起飞后安排任何旅游活动。
...
⚠️ 返程时间约束（极其重要）：
如果用户指定了返程起飞时间（[返程时间] 字段），最后一天的活动必须在「起飞时间 - 2小时」前全部结束。
例如：返程起飞 02:00（凌晨），则最后一天活动最晚安排到 23:59 前，且不能超过 00:00。
如果起飞时间在凌晨（00:00-05:59），最后一天的 evening 活动必须在前一天晚上 22:00 前结束，不能安排凌晨的游览活动。
绝对禁止在飞机起飞后安排任何旅游活动。

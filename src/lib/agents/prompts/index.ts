...
- 深夜/凌晨到达的当天：当晚 evening = []，不安排游览
- 返程起飞在凌晨（00:00-05:59）的当天：afternoon=[]，evening=[]，morning 只放前往机场
- 严禁在飞机起飞之后安排任何旅游或餐饮活动

⚠️ 返程时间约束（极其重要）：
如果 prompt 中有 [返程时间] 字段：
- 起飞在 00:00-11:59：最后一天 morning 只放前往机场，afternoon=[]，evening=[]
- 起飞在 12:00-17:59：最后一天 morning 可游览，afternoon 只放前往机场，evening=[]
- 起飞在 18:00-23:59：最后一天白天正常游览，evening 只放前往机场
...
- 深夜/凌晨到达的当天：当晚 evening 输出空数组 []，不安排游览
- 深夜/凌晨起飞的当天：如果起飞时间在凌晨（00:00-05:59），该天 afternoon 和 evening 均输出空数组 []，morning 只放前往机场/候机
- 如果某天只有部分时段有活动，其余时段直接用空数组 []，不要放"休息"或"自由活动"占位
- 严禁在飞机起飞之后安排任何旅游或餐饮活动

⚠️ 返程时间约束（极其重要）：
如果用户指定了返程起飞时间（[返程时间] 字段）：
- 起飞时间在 00:00-05:59（凌晨）：最后一天 morning 只放前往机场，afternoon=[]，evening=[]
- 起飞时间在 06:00-11:59（早上）：最后一天 morning 只放前往机场，afternoon=[]，evening=[]
- 起飞时间在 12:00-17:59（下午）：最后一天 morning 可有上午游览，afternoon 只放前往机场，evening=[]
- 起飞时间在 18:00-23:59（晚上）：最后一天正常安排白天，evening 只放前往机场
